...
export default function CategoryBlocks({
   ...
   const categoriesData = allCategories.map((cat) => {
      ...
      return { name: cat, originalName: originalCat, image: catMeta.image };
   });
         ...
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16 animate-in fade-in duration-700">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer flex flex-col gap-4 relative hover:opacity-80 transition-opacity">
      ...
      case 'brunch':
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12 animate-in fade-in duration-700">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer relative rounded-[2rem] overflow-hidden shadow-[0_4px_20px_-5px_rgba(6,81,237,0.05)] hover:shadow-[0_8px_30px_-5px_rgba(6,81,237,0.1)] transition-all hover:-translate-y-1 aspect-video border border-slate-50 bg-white">
                     {c.image ? (
      ...
      case 'cinematic':
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-24 animate-in fade-in duration-1000">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer overflow-hidden rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 p-3 transition-all duration-500 hover:bg-white/10 hover:border-amber-500/30 hover:shadow-[0_0_30px_rgba(245,158,11,0.15)] flex flex-col">
      ...
      case 'luxury':
         return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 sm:gap-16 mb-24 animate-in fade-in duration-1000">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer flex flex-col items-center justify-center text-center">
      ...
      case 'rustic':
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-20 animate-in fade-in duration-1000">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer border border-[#f0e6d5] p-5 bg-white rounded-t-[5rem] rounded-b-[2rem] shadow-sm hover:shadow-md hover:border-[#e9dac1] transition-all text-center">
      ...
      case 'sushi':
         return (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-16 animate-in fade-in duration-1000">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer border border-transparent hover:border-slate-800 p-2 sm:p-3 rounded-2xl transition-all relative overflow-hidden bg-slate-900/40">
      ...
      case 'taverna':
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16 animate-in fade-in duration-1000">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="group cursor-pointer border border-[#222] p-2 bg-[#111] transition-all hover:bg-[#1a1a1a]">
         ...
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16 animate-in slide-in-from-bottom-12 duration-500">
               {categoriesData.map((c, index) => {
                  ...
                  return (
                     <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className={`group cursor-pointer bg-white border-4 border-slate-900 rounded-3xl p-5 sm:p-6 shadow-[8px_8px_0px_#e2e8f0] hover:-translate-y-2 hover:-translate-x-2 transition-all flex flex-col relative ${hoverShadow}`}>
      ...
      default:
         return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
               {categoriesData.map((c) => (
                  <div key={c.name} onClick={() => onCategoryClick(c.originalName)} className="relative aspect-video sm:aspect-square rounded-3xl overflow-hidden cursor-pointer group shadow-2xl border border-[#222] bg-[#111]">
                     {c.image ? (
