...
export default function DashboardPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF8F5', color: B.carbone, fontFamily: "var(--font-inter), 'Inter', sans-serif", display: 'flex', transition: 'background .3s', overflowX: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(45,32,22,0.06)' }}>
          <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Image src="/sm-logo.svg" alt="SmartMenu Logo" width={40} height={40} style={{ objectFit: 'contain' }} />
            </div>
      ...
      {/* ═══════════════ MAIN CONTENT ═══════════════ */}
      <div style={{ flex: 1, marginLeft: isMobile ? 0 : 260, display: 'flex', flexDirection: 'column', minHeight: '100vh', transition: 'margin-left .3s' }}>

        {/* ── TOP BAR ─────────────────────────────────── */}
        <header style={{
          background: '#fff', borderBottom: '1px solid rgba(45,32,22,0.08)',
          position: 'sticky', top: 0, zIndex: 30,
          padding: isMobile ? '0 16px' : '0 32px', height: 64, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12,
        }}>
          {/* Mobile hamburger */}
          {isMobile && (
            <button
              ...
              aria-label="Menu"
              style={{
                background: 'none', border: '1px solid rgba(45,32,22,0.08)', borderRadius: 10,
                width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center',
                cursor: 'pointer', color: 'rgba(45,32,22,0.5)', flexShrink: 0,
              }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ position: 'relative' }} ref={dropdownRef}>
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: isMobile ? 0 : 8, padding: isMobile ? 4 : '4px 4px 4px 12px',
                  background: '#FAF8F5', border: '1px solid rgba(45,32,22,0.1)', borderRadius: 24,
                  cursor: 'pointer', transition: 'all .2s',
                }}
              >
                <span style={{ fontSize: 13, fontWeight: 500, color: 'rgba(45,32,22,0.6)', display: isMobile ? 'none' : 'block' }}>{user?.user_metadata?.full_name || user?.email?.split('@')[0]}</span>
                <div style={{
                  width: 32, height: 32, borderRadius: '50%', background: B.terracotta, color: B.crema,
                  fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0
                }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${isMobile ? '240px' : '280px'}, 1fr))`, gap: 16 }}>
                  {filteredMenus.map((menu) => (
                    <div key={menu.restaurant_id} onClick={() => router.push(`/admin?id=${menu.restaurant_id}`)} style={{
                      background: '#fff', border: '1px solid rgba(45,32,22,0.12)', borderRadius: 20,
                      overflow: 'hidden', cursor: 'pointer', transition: 'all .3s', display: 'flex', flexDirection: 'column',
                      boxShadow: '0 4px 12px rgba(45,32,22,0.04)',
                    }}
                      onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 8px 24px rgba(196,98,45,0.12)'; e.currentTarget.style.borderColor = 'rgba(196,98,45,0.3)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                      onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 4px 12px rgba(45,32,22,0.04)'; e.currentTarget.style.borderColor = 'rgba(45,32,22,0.12)'; e.currentTarget.style.transform = 'translateY(0)'; }}
                    >
                      {/* Cover */}
